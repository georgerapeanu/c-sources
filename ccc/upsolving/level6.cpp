...
struct power_provider_t {
    ...
    void set_max_power(int max_power, long long cost_limit){
        int st = 0, dr = max_power + 1;
...
bool ok(long long mid, vector<power_provider_t> &power_providers, vector<task_t> &tasks, int max_power, long long max_bill, int max_tasks) {
    ...
    for(int i = 0;i < (int)power_providers.size();i++){
        ...
        assert(power_providers[i].tasks == 0);
        assert(power_providers[i].power_limit <= max_power);
        assert(power_providers[i].price == input_power[id].price);
    }
    
    for(int i = 0;i < (int)tasks.size();i++){
        int id = tasks[i].id;
        task_t tmp = input_tasks[id - 1];
        //cerr << tasks[i].id << " " << tasks[i].power << " " << tasks[i].left << " " << tasks[i].right << endl; 
        //cerr << tmp.id << " " << tmp.power << " " << tmp.left << " " << tmp.right << endl; 
    ...
    for(auto &power_provider:power_providers) {
        ...
        int left_power = power_provider.power_limit - power_provider.power;
        int left_tasks = max_tasks - power_provider.tasks;
    
        for(auto &task:tasks) {
            if(left_power == 0 || left_tasks == 0){
                break;
            }
...
int main(int argc,char **argv) {
    ...
    while(dr - st > 1){
        ...
        if(ok(mid,power_providers,tasks,max_power,max_bill,max_tasks) == 0){
            st = mid;
        }
        else{
            dr = mid;
    ...
    cerr << dr << " " << ok(dr,power_providers,tasks,max_power,max_bill,max_tasks) << endl;
    long long bill = 0;
    for(auto power_provider:power_providers) {
