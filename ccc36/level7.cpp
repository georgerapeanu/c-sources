...
int main(){
  ...
  int last_layer = 0;
  for(;!states.empty();){
    state_t state = states.front();
  //  cerr << state.x << " " << state.y << " " << state.sum << "\n";
    states.pop_front();
    visited.erase(state);
    if(state.sum >= answer_sum){
      answer_sum = state.sum;
    }
    if(state.layer >= limit){
      continue;
    }
    while(state.layer + 1 > last_layer){
    ...
    for(int k = 0;k < (int)dir.size();k++){
      int xx = state.x + dir[k].first;
      int yy = state.y + dir[k].second;
      if(xx < 0 || xx >= (int)matrix.size()){
        continue;
      }
      if(yy < 0 || yy >= (int)matrix.size()){
      ...
      new_state.x = xx;
      new_state.y = yy;
      new_state.sum = state.sum + coin_costs[{xx, yy}];
      new_state.layer = state.layer + 1;
