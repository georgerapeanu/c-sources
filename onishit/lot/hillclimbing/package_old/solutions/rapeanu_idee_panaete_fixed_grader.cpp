...
long long solve(int n, long long k, int V[]){
    ...
    while(true){
      ...
      if(indexes[0] == (int)objects[0].size()){
        if(indexes[1] == (int)objects[1].size()){
          ...
          last_1_increase = objects[1][indexes[1]];
        }
      }else{
        if(indexes[1] == (int)objects[1].size()){
          ...
          if(objects[1][indexes[1]].increase_potential > 1){
            if(objects[0][indexes[0]].length() <= objects[1][indexes[1]].length() * 2) {
              ...
              long long increase = min((k / (objects[1][indexes[1]].length() * 2)), objects[1][indexes[1]].increase_potential / 2) * 2;
              k -= increase * objects[1][indexes[1]].length();
              objects[1][indexes[1]].increase_potential -= increase;
              ans += increase;
              if(increase == 0){
            ...
            if(indexes[1] + 1 >= (int)objects[1].size()) {
              ...
              long long increase = min((k / (objects[1][indexes[1]].length() + objects[1][indexes[1] + 1].length())), objects[1][indexes[1]].increase_potential);
              ...
              last_1_increase = objects[1][indexes[1] + 1];
            }
          }
        }
      }
    ...
    if(last_1_increase.l != -1){
      while(indexes[0] < (int)objects[0].size() && objects[0][indexes[0]].full()){
        indexes[0]++;
      }
      if(indexes[0] < (int)objects[0].size() && k + last_1_increase.length() >= objects[0][indexes[0]].length()){
        ans++;
      }
    }
