    ...
    for(int i = 1; i <= sum; ++i){
        ...
        while(j < l[i].size()){
            ...
            for(int k = mid - 1; k >= range[j].first; --k)
                gl[k] = b[l[i - 1][k]] | gl[k + 1];

            if(range[nxt].second > mid){
                gr[mid + 1] = b[l[i - 1][mid + 1]];
                for(int k = mid + 2; k <= range[nxt].second; ++k)
                    gr[k] = b[l[i - 1][k]] | gr[k - 1];
            }

            for(int k = j; k <= nxt; ++k){
                if(range[k].second == range[j].second){
                    b[l[i][k]] = gl[range[k].first] << p[l[i][k]][2];
                }
                else{
                    b[l[i][k]] = (gl[range[k].first] | gr[range[k].second]) << p[l[i][k]][2];
                }
            }
