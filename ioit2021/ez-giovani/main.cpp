...
const long double eps = 1e-15, inf = 1e9; 
...
int main() {
	...
	for (int i = 0; i < n ; ++i) {
		...
		for (int j = 0; j < 4 ; ++j)
		v.push_back(Halfplane(P[j], P[(j + 1) % 4]));
	}

	auto poly = hp_intersect(v); 

	ld area = 0;
	for (int i = 0, j = poly.size() - 1; i < poly.size() ; j = i, ++i) {
		area += (poly[j].x + poly[i].x) * (poly[j].y - poly[i].y); 
	}

	cout << fixed << setprecision(9) << abs(area / 2.0) << endl;
