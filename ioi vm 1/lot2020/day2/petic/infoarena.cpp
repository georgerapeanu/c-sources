...
FILE *f = fopen("petic.in","r");
FILE *g = fopen("petic.out","w");
...
int main() {
    ...
    vector<std::string> matrix(nrLin);
    vector<int> X1(Q);
    vector<int> Y1(Q);
    vector<int> X2(Q);
    vector<int> Y2(Q);
    ...
    vector<int> r = solve(nrLin, nrCol, Q, matrix, X1, Y1, X2, Y2);
