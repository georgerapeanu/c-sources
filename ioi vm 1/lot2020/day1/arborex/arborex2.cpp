...
void brut(int nod,int tata,int k){
    ...
    for(auto it:graph[nod]){
        ...
        brut(it.first,nod,k);
        sons[nod].push_back(brut_dp[it.first][1] + it.second);
    }
    sort(sons[nod].begin(),sons[nod].end());
    reverse(sons[nod].begin(),sons[nod].end());

    for(int i = 1;i < (int)sons[nod].size();i++){
        sons[nod][i] += sons[nod][i - 1];
    }

    for(int i = 0;i <= k && i <= (int)sons[nod].size();i++){
        int son_dp = (i > 0 ? sons[nod][i - 1]:0);
        int val1_dp = 0;
        int val2_dp = 0;
        if(vals[nod].size() == 0){
            ;
        }
        else if(k - i - 1 >= (int)prefsum[nod].size()){
            val1_dp = prefsum[nod].back();
        }
        else if(k - i - 1 >= 0){
            val1_dp = prefsum[nod][k - i - 1];
        }
        if(vals[nod].size() == 0){
            ;
        }
        else if(k - i - 2 >= (int)prefsum[nod].size()){
            val2_dp = prefsum[nod].back();
        }
        else if(k - i - 2 >= 0){
            val2_dp = prefsum[nod][k - i - 2];
        }
...
int dfs(int nod,int tata){
    ...
    if(special){
        for(auto it:possible_vals){///TODO just sort possible_vals, and then merge them or something, so you dodnt do O(n) each time here,idk
            vals[nod].push_back(it);
        }
        sort(vals[nod].begin(),vals[nod].end());
        reverse(vals[nod].begin(),vals[nod].end());
        if(vals[nod].size()){
            prefsum[nod] = {vals[nod][0]};
        }
        for(int i = 1;i < (int)vals[nod].size();i++){
            prefsum[nod].push_back(vals[nod][i] + prefsum[nod][i - 1]);
        }
        for(auto it:roots){
            int to = it.first;
